...
criterion = nn.ParallelCriterion():add(nn.MSECriterion(),1):add(nn.SmoothAndEdgeTerm(0.1,7,2,0.8,1,0.1,20,0,0),1)
...
postfix = 'smooth_texture-removal'
max_iters = 30
batch_size = 1
...
local file = './smoothing_codes/train_texture-removal.lua'
  ...
  for t = 1,trainsetSize,batch_size do
    ...
    local feval = function(x_new)
      ...
      for i = 1,size do
        ...
        local inputEdgeFile = string.gsub(inputFile,'VOC2012_input','VOC2012_input_edge_texture')
        inputEdgeFileLabelSaliency = string.gsub(inputEdgeFile,'%.png','-edge.png')
        ...
        local saliency = image.load(inputEdgeFileLabelSaliency)
