...
criterion = nn.ParallelCriterion():add(nn.MSECriterion(),1):add(nn.SmoothAndEdgeTerm(0.1,7,1,0.8,1,0.1,5,0,0),1)
...
postfix = 'smooth_foreground-enhance'
max_iters = 30
batch_size = 1
...
local file = './smoothing_codes/train_foreground-enhance.lua'
  ...
  for t = 1,trainsetSize,batch_size do
    ...
    local feval = function(x_new)
      ...
      for i = 1,size do
        ...
        local inputSaliency = string.gsub(inputFile,'VOC2012_input','VOC2012_input_saliency')
        ...
        local input_edge = model_computeEdge:forward(input)
        local saliency = image.load(inputSaliency)
        local edgeLabel = image.load(inputEdgeFile)
        saliency = torch.lt(saliency,0.5)
        saliency = saliency:cuda()
        edgeLabel = edgeLabel:cuda()
        ...
        label_all[{{},{4},{},{}}] = torch.cmul(saliency,input_edge)
        label_all[{{},{5},{},{}}] = torch.cmul(saliency,edgeLabel)
